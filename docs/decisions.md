...
- The saved `processed_mask_path` now contains the object foreground mask, not the broader raw-valid-depth mask, so patch training and heatmaps ignore valid background pixels inside the crop.
...
- Classical training now starts with bounded-sample, per-category `OneClassSVM` baselines: each object category gets its own scaler and model trained only from that category's normal training patches.
- Classical inference loads the matching category-specific model for every test image, aggregates patch scores with top-k mean for image-level ranking, and saves raw overlap-averaged heatmaps separately from display-normalized qualitative figures.
- Classical heatmap figures now transform dataset-provided `gt/*.png` defect annotations with the same crop/resize geometry as the depth map and draw them as cyan contours over depth, heatmap, and overlay panels.
- The first nine-value handcrafted classical feature set is no longer the main baseline because it removes too much spatial depth structure. The classical pipeline now uses the normalized patch height values themselves as the feature vector, with optional PCA before the category-specific `OneClassSVM`, so it remains directly comparable to the later autoencoder input while keeping a classical density/boundary model.
